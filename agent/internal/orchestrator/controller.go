...
type Shutdowner interface { Shutdown(context.Context, config.HostConfig) (host.ShutdownResult, error) }
...
func (c *Controller) Tick(ctx context.Context, now time.Time, in policy.Inputs) (policy.Decision, error) {
	if c.Policy == nil { return policy.Decision{}, errors.New("policy coordinator is required") }
	before := c.Policy.State().PowerState
	decision, err := c.Policy.Step(now, in); if err != nil { return policy.Decision{}, err }
	after := c.Policy.State().PowerState
	if before == state.Normal && after == state.OnBattery {
		if err := c.beginFreshOutage(ctx); err != nil { return policy.Decision{}, err }
	} else if before == state.BootReconcile && after == state.OnBattery {
		if err := c.snapshotHosts(ctx, false); err != nil { return policy.Decision{}, err }
	}
	switch decision.Action {
	case policy.ActionCommitShutdown:
		// Persist SHUTDOWN_IN_PROGRESS before the first per-host side effect.
		if _, err := c.Policy.Step(now, in); err != nil { return policy.Decision{}, err }
		if err := c.executeShutdown(ctx); err != nil { return decision, err }
...
func (c *Controller) beginFreshOutage(ctx context.Context) error {
	...
	if c.NewTransactionID != nil {
		id := c.NewTransactionID(); if id == "" { return errors.New("new transaction id is empty") }
		st.ParentTransactionID = st.TransactionID; st.TransactionID = id
	}
	st.Hosts = map[string]state.HostState{}
	st.ShutdownCommitted = false; st.RecoveryStarted = false; st.FailedSafeReason = ""
	if _, err := c.Policy.Write(st); err != nil { return fmt.Errorf("persist new outage transaction: %w", err) }
	return c.snapshotHosts(ctx, true)
}

func (c *Controller) snapshotHosts(ctx context.Context, offlineKnown bool) error {
	if c.Probe == nil { return errors.New("host prober is required") }
	st := cloneState(c.Policy.State())
	for _, h := range c.Config.Hosts {
		hs, exists := st.Hosts[h.ID]
		if exists && hs.WasOnline != nil { continue }
		hs.ShutdownState = state.ShutdownPlanned
		if h.Shutdown.Method == "none" { hs.ShutdownState = state.ShutdownNotRequired }
		hs.RecoveryState = state.RecoveryWaiting
		if !h.Wake.Enabled || h.RestorePolicy == "never" { hs.RecoveryState = state.RecoveryNotRequired }
		if h.Address == nil || h.Status.Method == "none" {
			hs.WasOnline = nil; hs.LastVerification = "unknown"
		} else {
			result, err := c.Probe.Check(ctx, h)
			if err != nil { hs.WasOnline=nil; hs.LastVerification="unknown"; hs.LastError=err.Error() } else if result.Known && result.Online { v:=true; hs.WasOnline=&v; hs.LastVerification="online" } else if result.Known && offlineKnown { v:=false; hs.WasOnline=&v; hs.LastVerification="offline" } else { hs.WasOnline=nil; hs.LastVerification="unknown" }
		}
		st.Hosts[h.ID] = hs
	}
	_, err := c.Policy.Write(st); if err != nil { return fmt.Errorf("persist outage snapshot: %w", err) }; return nil
}
...
func (c *Controller) executeShutdown(ctx context.Context) error {
	...
	if len(plan.NUTGroup)>0 {
		...
		if err := c.prepareNUTGroup(ctx, plan.NUTGroup, byID); err != nil { return err }
		if err := c.FSD.RequestFSD(ctx, c.Config.NUT.UPSName, c.upsmonPath()); err != nil { c.markNUTError(plan.NUTGroup, err); return fmt.Errorf("request FSD: %w", err) }
		c.markNUTAcknowledged(plan.NUTGroup)
		c.verifyNUTGroup(ctx, plan.NUTGroup, byID)
	}
	_, err := c.Policy.MarkShutdownPhaseComplete(); return err
}
...
func (c *Controller) prepareNUTGroup(ctx context.Context, ids []string, byID map[string]config.HostConfig) error {
	for _,id:=range ids{h:=byID[id];hs:=c.Policy.State().Hosts[id];if hs.ShutdownState==state.ShutdownCompleted{continue};if h.Address!=nil&&h.Status.Method!="none"{res,err:=c.Probe.Check(ctx,h);if err==nil&&res.Known&&!res.Online{hs.ShutdownState=state.ShutdownCompleted;hs.LastVerification="offline";if err:=c.persistHost(id,hs);err!=nil{return err};continue}};hs.ShutdownAttempts++;hs.ShutdownState=state.ShutdownRequested;hs.LastActionID=fmt.Sprintf("fsd:%s:%s:%d",c.Policy.State().TransactionID,id,hs.ShutdownAttempts);if err:=c.persistHost(id,hs);err!=nil{return err}}
	return nil
}
func(c *Controller)markNUTAcknowledged(ids []string){for _,id:=range ids{hs:=c.Policy.State().Hosts[id];if hs.ShutdownState==state.ShutdownRequested{hs.ShutdownState=state.ShutdownAcknowledged;hs.LastError="";_ = c.persistHost(id,hs)}}}
func(c *Controller)markNUTError(ids []string,cause error){for _,id:=range ids{hs:=c.Policy.State().Hosts[id];if hs.ShutdownState!=state.ShutdownCompleted{hs.ShutdownState=state.ShutdownUnknown;hs.LastError=cause.Error();_ = c.persistHost(id,hs)}}}
func(c *Controller)verifyNUTGroup(ctx context.Context,ids []string,byID map[string]config.HostConfig){for _,id:=range ids{h:=byID[id];hs:=c.Policy.State().Hosts[id];if hs.ShutdownState==state.ShutdownCompleted{continue};if h.Address==nil||h.Status.Method=="none"{continue};waitCtx,cancel:=context.WithTimeout(ctx,time.Duration(h.Shutdown.TimeoutSeconds)*time.Second);err:=c.Probe.Wait(waitCtx,h,false);cancel();hs=c.Policy.State().Hosts[id];if err!=nil{hs.ShutdownState=state.ShutdownUnknown;hs.LastError=err.Error();hs.LastVerification="unknown"}else{hs.ShutdownState=state.ShutdownCompleted;hs.LastError="";hs.LastVerification="offline"};_ = c.persistHost(id,hs)}}

func (c *Controller) executeRecoveryStep(ctx context.Context) error {
	if c.Recovery == nil { return errors.New("recovery runner is required") }
	st:=c.Policy.State();hosts:=toHostConfigs(c.Config.Hosts)
	if markBlockedRecovery(&st,hosts) { if _,err:=c.Policy.Write(st);err!=nil{return err};st=c.Policy.State() }
	if recoverySettled(hosts,st.Hosts) { _,err:=c.Policy.MarkRecoveryComplete();return err }
	updated,id,err:=c.Recovery.RunNext(ctx,cloneState(st),hosts);_ = updated
	if err!=nil { // a per-host failed wake is durable; continue independent hosts on later ticks
		if id!="" { return nil }; return err
	}
	if id=="" && recoverySettled(hosts,c.Policy.State().Hosts) { _,err:=c.Policy.MarkRecoveryComplete();return err }
	return nil
}

func (c *Controller) persistHost(id string, hs state.HostState) error { st:=cloneState(c.Policy.State());st.Hosts[id]=hs;_,err:=c.Policy.Write(st);return err }
func (c *Controller) upsmonPath() string { if c.UPSMonConfPath!=""{return c.UPSMonConfPath};return "/etc/nut/upsmon.conf" }
func cloneState(in state.State) state.State { out:=in;out.Hosts=make(map[string]state.HostState,len(in.Hosts));for k,v:=range in.Hosts{out.Hosts[k]=v};return out }
func toHostConfigs(in []config.HostConfig) []host.Config { out:=make([]host.Config,0,len(in));for _,h:=range in{hc:=host.Config{ID:h.ID,ShutdownMethod:h.Shutdown.Method,ShutdownPriority:h.Shutdown.Priority,WakeEnabled:h.Wake.Enabled,WakePriority:h.Wake.Priority,WakeMaxAttempts:h.Wake.MaxAttempts,WakePort:h.Wake.Port,RestorePolicy:host.RestorePolicy(h.RestorePolicy),DependsOn:append([]string(nil),h.DependsOn...)};if h.Wake.MAC!=nil{hc.WakeMAC=*h.Wake.MAC};if h.Wake.Interface!=nil{hc.WakeInterface=*h.Wake.Interface};if h.Wake.Broadcast!=nil{hc.WakeBroadcast=*h.Wake.Broadcast};out=append(out,hc)};return out }
func recoverySettled(hosts []host.Config, states map[string]state.HostState) bool { any:=false;for _,h:=range hosts{hs,ok:=states[h.ID];if !ok||!host.EligibleForRestore(h,hs){continue};any=true;if hs.RecoveryState!=state.RecoveryOnline&&hs.RecoveryState!=state.RecoveryFailed&&hs.RecoveryState!=state.RecoveryNotRequired{return false}};return any||true }
func markBlockedRecovery(st *state.State,hosts []host.Config) bool { byID:=map[string]host.Config{};for _,h:=range hosts{byID[h.ID]=h};changed:=false;for _,h:=range hosts{hs:=st.Hosts[h.ID];if !host.EligibleForRestore(h,hs)||hs.RecoveryState==state.RecoveryOnline||hs.RecoveryState==state.RecoveryFailed{continue};for _,dep:=range h.DependsOn{dcfg,ok:=byID[dep];if !ok{continue};ds:=st.Hosts[dep];if host.EligibleForRestore(dcfg,ds)&&ds.RecoveryState==state.RecoveryFailed{hs.RecoveryState=state.RecoveryFailed;hs.LastError="managed recovery dependency failed: "+dep;st.Hosts[h.ID]=hs;changed=true;break}}};return changed }
