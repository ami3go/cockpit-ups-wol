...
type fakeCheck struct{ name string; result Result }
func (f fakeCheck) Name() string { return f.name }
func (f fakeCheck) Run(context.Context) Result { r:=f.result; if r.Name==""{r.Name=f.name}; return r }

type fakeRepair struct{ name string; calls int; err error }
...
func TestHealthySnapshot(t *testing.T){
	...
	snap,err:=s.Run(context.Background(),true);if err!=nil{t.Fatal(err)};if snap.State!=Healthy{t.Fatalf("state=%s",snap.State)}
}

func TestRepairCircuitPersistsAcrossRuns(t *testing.T){
	now:=time.Date(2026,9,19,12,0,0,0,time.UTC);repair:=&fakeRepair{name:"systemd:agent",err:errors.New("still broken")};path:=filepath.Join(t.TempDir(),"health.json")
	...
	first,err:=s.Run(context.Background(),true);if err!=nil{t.Fatal(err)};if first.Circuit.Attempts["systemd:agent"]!=1||repair.calls!=1{t.Fatalf("first=%+v calls=%d",first,repair.calls)}
	...
	second,err:=s2.Run(context.Background(),true);if err!=nil{t.Fatal(err)};if second.State!=FailedSafe{t.Fatalf("state=%s",second.State)};if second.Circuit.Attempts["systemd:agent"]!=2{t.Fatalf("attempts=%d",second.Circuit.Attempts["systemd:agent"])}
}
...
func TestUnregisteredRepairNeverRunsArbitraryAction(t *testing.T){
	...
	snap,err:=s.Run(context.Background(),true);if err!=nil{t.Fatal(err)};if len(snap.Repairs)!=0{t.Fatalf("repairs=%v",snap.Repairs)};if snap.State!=Degraded{t.Fatalf("state=%s",snap.State)}
}
