...
func (s *Server) serveConn(ctx context.Context, conn net.Conn) {
	...
	if len(line) > MaxRequestBytes {
		_ = json.NewEncoder(conn).Encode(failure("", "INVALID_REQUEST", "request too large"))
		return
	}
	select {
	case <-ctx.Done():
	...
	if err != nil {
		_ = json.NewEncoder(conn).Encode(failure("", "INVALID_REQUEST", err.Error()))
		return
	}
...
func Call(ctx context.Context, socketPath string, req Request) (Response, error) {
	...
	if resp.ID != req.ID && resp.ID != "" {
		return resp, fmt.Errorf("response id mismatch: got %q want %q", resp.ID, req.ID)
	}
	return resp, nil
}
