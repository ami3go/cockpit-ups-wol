...
func TestSSHShutdownUsesFixedArgumentVector(t *testing.T) {
	...
	res, err := (ShutdownExecutor{Runner: r}).Shutdown(context.Background(), h)
	if err != nil {
		t.Fatal(err)
	}
	if res.Disposition != ShutdownDirectRequested {
		t.Fatalf("unexpected disposition %s", res.Disposition)
	}
	want := []string{
		"-o", "BatchMode=yes",
	...
	if r.name != "ssh" || !reflect.DeepEqual(r.args, want) {
		t.Fatalf("unexpected exec: %s %#v", r.name, r.args)
	}
}
...
func TestSSHRejectsOptionLikeAddress(t *testing.T) {
	...
	if _, err := (ShutdownExecutor{Runner: r}).Shutdown(context.Background(), h); err == nil {
...
func TestNUTAndCommandShutdownSemantics(t *testing.T) {
	res, err := (ShutdownExecutor{}).Shutdown(context.Background(), config.HostConfig{Shutdown: config.ShutdownConfig{Method: "nut"}})
	if err != nil || res.Disposition != ShutdownManagedByNUT {
		t.Fatalf("NUT result=%+v err=%v", res, err)
	}
	if _, err := (ShutdownExecutor{}).Shutdown(context.Background(), config.HostConfig{Shutdown: config.ShutdownConfig{Method: "command"}}); err == nil {
		t.Fatal("command shutdown must fail closed")
	}
}
...
func TestTCPStatus(t *testing.T) {
	...
	res, err := (StatusChecker{}).Check(context.Background(), "127.0.0.1", config.StatusConfig{Method: "tcp", Port: intp(port), TimeoutMS: 500})
	if err != nil {
		t.Fatal(err)
	}
	if !res.Known || !res.Online {
...
func TestArmedCapabilitiesRejectUnsupportedCommandAndARP(t *testing.T) {
	...
	if err := ValidateArmedCapabilities(cfg); err == nil {
		t.Fatal("expected command rejection")
	}
	cfg.Hosts[0].Shutdown.Method = "none"
	cfg.Hosts[0].Status.Method = "arp"
	if err := ValidateArmedCapabilities(cfg); err == nil {
		t.Fatal("expected arp rejection")
	}
}
