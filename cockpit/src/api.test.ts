...
function installCockpit(response = "{}") {
  ...
  const spawn = vi.fn((command: string[], options?: CockpitSpawnOptions) => {
    ...
    promise.input = (data: string | null | undefined) => {
      call.input = data ?? undefined;
    };
    return promise;
  });
...
describe("Cockpit command privilege boundary", () => {
  it("uses try privilege for read-only commands", async () => {
    const { calls } = installCockpit(
      '{"ok":true,"result":{"state":"HEALTHY","checked_at":"now","results":[],"generation":1}}',
    );
    await getHealth();
    await getConfigStatus();
    await getPlan();
    await getLogs();
    expect(calls.map((c) => c.options?.superuser)).toEqual([
      "try",
      "try",
      "try",
      "try",
  ...
  it("passes candidate config on stdin while validation stays read-only", async () => {
    ...
    expect(calls[0].command[calls[0].command.length - 1]).toBe(
      "config-validate",
    );
