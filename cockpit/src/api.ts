...
export interface HealthResponse { ok: boolean; data?: HealthSnapshot; error?: { message?: string }; }
export interface ConfigStatus { active: string; last_known_good: string; previous_known_good: string; active_manifest?: { status: string; created_at: string }; last_known_good_manifest?: { status: string; created_at: string }; }
...
async function jsonCommand<T>(args: string[]): Promise<T> {
  const text = await window.cockpit.spawn([ctl, ...args], { err: 'message' });
...
export const getLogs = () => window.cockpit.spawn([ctl, 'logs'], { err: 'message' });
