...
cleanup(){ sudo upsdrvctl stop ups >/dev/null 2>&1||true; sudo pkill -x upsd >/dev/null 2>&1||true; }
trap cleanup EXIT
...
sudo systemctl stop nut-server.service nut-monitor.service >/dev/null 2>&1||true
cleanup
