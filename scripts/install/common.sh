...
validate_test_overrides(){
  local v
  if [[ "${COCKPIT_UPS_WOL_TEST_MODE:-0}" == 1 ]]; then
    printf 'WARN: cockpit-ups-wol installer TEST MODE enabled; test-only environment overrides are active.\n' >&2
    return 0
  fi
  for v in COCKPIT_UPS_WOL_COCKPIT_DIR COCKPIT_UPS_WOL_NUT_ETC_DIR COCKPIT_UPS_WOL_NUT_SCAN_FIXTURE COCKPIT_UPS_WOL_PROBATION_SECONDS COCKPIT_UPS_WOL_REQUIRE_UI; do
    if [[ -n "${!v:-}" ]]; then
      die "$v is a test-only installer override; set COCKPIT_UPS_WOL_TEST_MODE=1 only in controlled tests"
    fi
  done
}
...
install_project_dirs(){ install -d -m 0755 "$LIBEXEC_DIR"; install -d -m 0750 "$ETC_DIR"; install -d -m 0700 "$ETC_DIR/secrets" "$STATE_DIR" "$STATE_DIR/config-history/revisions"; if [[ ! -e "$ETC_DIR/known_hosts" ]]; then install -m0644 /dev/null "$ETC_DIR/known_hosts"; else chmod 0644 "$ETC_DIR/known_hosts"; fi; }
